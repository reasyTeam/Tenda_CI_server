    ...
    export default {
        ...
        methods: {
            ...
            getAllDatas: function() {
                ...
                Promise
                    .all([
                        this.$http.post("/api/CI/getAllProducts"),
                        this.$http.post("/api/CI/getProLine")
                    ])
                    .then((res) => {
                        that.isLoadingData = false;
                        that.tableData = res[0].data.products;
                        that.allMembers = res[1].data.allMembers;
                        that.productLines = res[1].data.productLines.map(arr => arr.productLine);
                    });
            },
            submit: function() {
                ...
                this.$http
                    .post("/api/CI/editProduct", submitData)
                    .then(res => {
                        that.dialogLoading = false;
                        that.notify(res.data);
                        if (res.data.status == "ok") {
